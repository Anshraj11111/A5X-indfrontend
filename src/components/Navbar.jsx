      ...
      <AnimatePresence>
        {open && (
          <>
            ...
            {/* Drawer */}
            <motion.div
              className="fixed top-0 right-0 w-full h-full bg-[#050b11] z-[9999]
                         pt-28 px-8 overflow-y-auto"
              initial={{ x: "100%" }}
              animate={{ x: 0 }}
              exit={{ x: "100%" }}
              transition={{ duration: 0.3 }}
            >
              {/* ✅ Drawer Content */}
              <div className="flex flex-col gap-6 text-2xl font-extrabold tracking-wide pb-10">
                {navItems.map(({ to, label }) => (
                  <NavLink
                    key={to}
                    to={to}
                ...
                <NavLink
                  to="/admin/login"
                  className="mt-4 text-[#00ffff] border border-[#00ffff]
                             px-6 py-3 rounded-full text-center
                             hover:bg-[#00ffff] hover:text-black transition"
                >
                  Admin
                ...
                {/* ✅ Footer stays visible now because scroll enabled */}
