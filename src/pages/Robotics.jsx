    ...
    <main className="pt-24 bg-black text-white min-h-screen">
      ...
      <section className="relative min-h-[70vh] md:min-h-[80vh] flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 text-center px-4 sm:px-6 max-w-3xl mx-auto">
          <h1 className="robotics-title text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-bold leading-tight tracking-tight">
            Robotics Designed for<br />Real-World Application
          </h1>

          <p className="robotics-sub mt-6 text-gray-300 text-base sm:text-lg md:text-xl max-w-2xl mx-auto">
      ...
      <section className="py-20 bg-[#05080F] border-t border-white/10">
        <div className="max-w-7xl mx-auto px-6">
          ...
          <p className="mt-4 text-center text-gray-400 max-w-2xl mx-auto text-base sm:text-lg">
            Built like competition machines — not prototypes.
          </p>

          <div className="mt-12 md:mt-14 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-10">
            {[
              {
                img: droneImg,
                title: "Palm Size Drone",
              ...
              <div
                key={p.title}
                className="robotics-card rounded-3xl overflow-hidden border border-cyan-400/40
                           bg-[#061018]/70 backdrop-blur-lg
                           hover:shadow-[0_0_40px_rgba(0,255,255,0.5)] transition duration-300 flex flex-col"
              >
                <img src={p.img} alt={p.title} className="w-full h-48 sm:h-56 md:h-60 object-cover" />
                <div className="p-5 sm:p-6 flex-1">
                  <h3 className="text-lg sm:text-xl md:text-2xl font-bold">{p.title}</h3>
                  <p className="mt-3 text-gray-300 text-sm md:text-base leading-relaxed">{p.desc}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="py-16 md:py-20 bg-[#060C12]">
        <div className="max-w-6xl mx-auto px-6 text-center">
          ...
          <div className="mt-10 flex flex-wrap gap-3 md:gap-4 justify-center">
            {[
              "PID",
              "STM32",
              "ESP32",
              ...
              <span
                key={s}
                className="robotics-badge px-4 md:px-5 py-2 rounded-full border border-cyan-400/60 text-cyan-400
                           bg-black/30 hover:bg-cyan-400/10 hover:text-cyan-300 transition duration-300 text-sm md:text-base"
              >
                {s}
              </span>
            ))}
